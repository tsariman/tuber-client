...
export default class StateThemeParser {
  ...
  getParser (): (theme: Theme, rules: Record<string, unknown>) => TO {
   ...
   * ```
   */
  private _deleteProperties (propertyBin: string[], rules: TO) {
  ...
  private _parse (rules : TO): TO {
    ...
    for (const prop in rules) {
      ...
      if (typeof val === 'object' && !Array.isArray(val)) {
        rules[prop] = this._parse(val as TO)
      }
