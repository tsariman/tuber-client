...
export default function RecursiveFormItems (props: IRecursiveFormBuilder) {
  ...
  const groupItem = useCallback((def: StateFormItem, key: string|number) => {
    ...
    return (
      <StateJsxFormItemGroup key={`group${depth}-${key}`} instance={groupItemDef}>
        <RecursiveFormItems
  ...
  const itemsTable: IItemTable = useMemo(() => ({
    ...
    [CHECKBOXES]: (def: StateFormItem, key: string|number) => (
      <DialogCheckboxes
        instance={def as StateFormItem<StateForm, StateFormItemCheckboxBox>}
        hive={hive}
    ...
    [STATE_SELECT]: (def: StateFormItem, key: string|number) => (
      <DialogSelect
        instance={def as StateFormItem<StateForm, IStateFormItemSelectOption>}
        hive={hive}
